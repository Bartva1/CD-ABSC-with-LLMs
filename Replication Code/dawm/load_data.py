...
class CustomDataset2(Dataset):
    def __init__(self, *tensors, target_tensor_index, target_values, indices=None):
        self.tensors = list(tensors)
        self.target_tensor_index = target_tensor_index
        self.target_values = target_values
        assert all(len(t) == len(tensors[0]) for t in tensors), "Length of tensors must be the same"

        # Filter indices where the domain tensor has one of the target values (source or target)
        self.filtered_indices = self._filter_indices()

        # If indices are provided, use only those
        if indices is not None:
            self.filtered_indices = self.filtered_indices[indices]

        # Build domain_tensor and hot_domain
        self.domain_tensor = []
        self.hot_domain = []
        target_tensor = self.tensors[self.target_tensor_index]

        for val in target_tensor:
            if val == target_values[0]:  # source
                self.domain_tensor.append(0)
                self.hot_domain.append([1.0, 0.0])
            elif len(target_values) > 1 and val == target_values[1]:  # target
                self.domain_tensor.append(1)
                self.hot_domain.append([0.0, 1.0])
            else:
                self.domain_tensor.append(2)  # neither source nor target (filtered out later)
    ...
    def __getitem__(self, idx):
        ...
        # Update domain_tensor and hot_domain into the tensors
        self.tensors[self.target_tensor_index] = self.domain_tensor
        self.tensors[4] = self.hot_domain  # assumes index 4 is reserved for hot_domain
...
def load_data(file_path,index, train_domain):
    ...
    with open(file_path, 'r' , encoding='latin-1') as file:
        ...
        for i in range(0+index*999, d, 3):
            sample = (lines[i].strip(), lines[i+1].strip(), lines[i+2].strip())
    ...
    for idx in range(len(samples)):
        first_element, second_element,third_element = samples[idx]
        ...
        if pol == -1:
            ...
            print('error, unknown polarity')
        
        
        domain_map = {"laptop": [1.0,0.0,0.0],
                      "restaurant": [0.0,1.0,0.0],
                      "book": [0.0,1.0,0.0]}
        
        if train_domain in domain_map:
            domain.append(domain_map[train_domain])
        else:
            print(f"error: got wrong domain: {train_domain}")
    ...
    mask_constraints = torch.ones((len(sentences), MAX_LENGTH), dtype=torch.int).cpu()
